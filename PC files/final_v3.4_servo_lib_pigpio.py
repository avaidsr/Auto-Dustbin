...
servo_off = 0 #pulsewidth 0 for motor off
servo_org = 1900 #pulsewidth 1900 for organic position
servo_rec = 500 #pulsewidth 500 for recyclable position
servo_middle = 1200 #pulsewidth 1200 for center position
...
pi = pigpio.pi() #for servo initialization
pi.set_servo_pulsewidth(servo, servo_middle) # Initialize at center position
...
classifier = load_model('recycle_model_V4_GC_3C_E90_B512.h5')
...
def input_detection_identificaion():
    ...
    new_result = np.argmax(result)
    print(np.argmax(result))
    if 0<=new_result<=119:
        #class - Organic
        new_result = 1
    elif 120<=new_result<=124:
        #class - Recyclable
        new_result = 0
...
def comp():
    ...
    # if correct input - perform following actions
        ...
        if(new_result == 1):
            print('Organic')
            print('Start servo')
            pi.set_servo_pulsewidth(servo, servo_org)
            print('Servo moved')
            print('sleep')
            time.sleep(2)
            print('sleep finished')
            pi.set_servo_pulsewidth(servo, servo_middle)
            print('servo back to default location 90')
        elif(new_result == 0):
            print('Recyclable')
            print('Start servo')
            pi.set_servo_pulsewidth(servo, servo_rec)
            print('Servo moved')
            print('sleep')
            time.sleep(2)
            print('sleep finished')
